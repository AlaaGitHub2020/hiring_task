import 'package:flutter/material.dart';
import 'package:hiring_task/application/albums/albums_details_bloc.dart';
import 'package:hiring_task/domain/core/utilities/colors.dart';
import 'package:hiring_task/generated/l10n.dart';

class AlbumDetailsCustomErrorWidget extends StatelessWidget {
  final AlbumDetailsFailure failure;

  const AlbumDetailsCustomErrorWidget({Key? key, required this.failure})
      : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Center(
      child: Text(
        failure.albumFailure.maybeMap(
          getAlbumDetailsFailure: (_) => S.of(context).getAlbumDetailsFailure,
          orElse: () => S.of(context).unexpected,
        ),
        style: Theme.of(context)
            .textTheme
            .headline1!
            .copyWith(color: AppColors.redColor1),
        textAlign: TextAlign.center,
      ),
    );
  }
}
