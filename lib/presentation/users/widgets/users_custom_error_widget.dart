import 'package:flutter/material.dart';
import 'package:hiring_task/application/user/users_list/users_list_bloc.dart';
import 'package:hiring_task/domain/core/utilities/colors.dart';
import 'package:hiring_task/generated/l10n.dart';

class UsersCustomErrorWidget extends StatelessWidget {
  final UsersListFailure failure;

  const UsersCustomErrorWidget({Key? key, required this.failure})
      : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Center(
      child: Text(
        failure.userFailure.maybeMap(
          fetchUsersListFailure: (_) => S.of(context).fetchUsersListFailure,
          orElse: () => S.of(context).unexpected,
        ),
        style: Theme.of(context)
            .textTheme
            .headline1!
            .copyWith(color: AppColors.redColor1),
        textAlign: TextAlign.center,
      ),
    );
  }
}
